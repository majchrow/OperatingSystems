...
void exit_handler();
void try_connect();
void send_msg(uint8_t tag, char* value);
...
int main(int argc, char *argv[])
{
    ...
    while(1){
        ...
        while (TRUE) {
            ...
            switch (tag) {
                case REQUEST:
                    if (read(client_fd, &req, sizeof(struct request)) < 0) {
...
void init(int argc, char *argv[]){
    ...
    if(strcmp(argv[2], "Web") == 0){ // Web connection initializing
        ...
        if((client_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0){ // IPv4 + Stream
        ...
        if((client_fd = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0){ // Unix path + Stream
            fprintf(stderr, "Client %-10s: Failed to create local socket\n", client_name);
            exit(EXIT_FAILURE);
        }


//        bind(client_fd, (struct sockaddr *)&local_serv_addr, sizeof(local_serv_addr));
...
void try_connect(){
    ...
    if( read(client_fd, &response, TAG) != TAG){
...
void send_msg(uint8_t tag, char* value){
    ...
    msg.type = tag;
    if(value){
        strcpy(msg.content, value);
    }
    pthread_mutex_lock(&mutex);
    if (write(client_fd, &msg, sizeof(msg)) != sizeof(msg)){
        fprintf(stderr, "Client %-10s: Failed to send message to the server\n", client_name);
        exit(EXIT_FAILURE);
    }
